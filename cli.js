...
require('sywac')
  .preface(
    chalk.magenta(figlet.textSync('baasha', 'Univers')),
  ...
  .help('-h, --help')
  .version('-v, --version')
  .example('baasha', { desc: 'Prints one product name and done' })
  .example('baasha -e 10', { desc: 'Prints ten product names, one second apart' })
  .example('baasha -n 5 -x 20 -i 2000', { desc: 'Prints between 5 and 20 product names, 2 seconds apart' })
  .example('baasha -fm', { desc: 'Perpetually prints a new name on separate lines' })
  .style({
  ...
  .then(argv => {
    ...
    function nextName () {
      setTimeout(() => {
        const name = argv.f ? baasha.sync({ exactly: 1, object: true }) : names.shift()
